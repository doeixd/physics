...
\paragraph{Apparatus:} Microwave cavity mode serving as readout resonator, with controllable quantum state.

\paragraph{Experimental sequence:}
\begin{enumerate}
    \item \textbf{Preparation.} Cool cavity to ground state ($\sim 20$ mK).
    \item \textbf{Squeezing.} Apply parametric drive to generate squeezed state $\ket{r, 0}$ with $r \approx 1$. Verify squeezing via homodyne tomography: $(\Delta X)^2 = e^{-2r}/2$.
    \item \textbf{Measurement.} Trigger qubit-cavity interaction, driving cavity to pointer states $\ket{\alpha_0}$ (qubit in $\ket{0}$) or $\ket{\alpha_1}$ (qubit in $\ket{1}$).
    \item \textbf{Readout.} Perform heterodyne detection of cavity state. Record outcome.
    \item \textbf{Repeat.} Perform $N_{\text{trials}} = 10^4$ measurements.
\end{enumerate}
...
\paragraph{Feasibility:} High. Squeezing at $r \sim 1$ routinely achieved in circuit QED (e.g., MIT, Yale, JILA groups). Estimated timeline: 2-3 years. Estimated cost: \$500k-1M (typical superconducting qubit experiment).

\paragraph{Collaborators:} Discussions initiated with optomechanics groups at MIT-RLE and JILA (preliminary interest confirmed).
